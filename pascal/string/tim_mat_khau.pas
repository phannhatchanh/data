{
  Việc bảo vệ máy tính để hạn chế người khác thâm nhập là một vấn đề đặt ra cho mọi người sử dụng máy tính.
Để tăng tính an toàn trong lưu trữ, một người đã quyết định giấu mật khẩu truy cập máy tính của mình vào
một xâu S với một qui ước sao cho khi cần anh ta có thể lấy lại được mật khẩu từ xâu S. Là một người yêu 
thích số học anh ta chọn mật khẩu P là một số tự nhiên và mã hóa P=P1+P2+...+Pi (Với Pi là số nguyên, 0<=Pi<=9 và 0<i<200)
Đem dấu các số Pi vào một xâu kí tự S.
  Chẳng hạn xâu S='mat #223 ma 678 @' chứa mật khẩu là 28 vì P=2+2+3+6+7+8
  Yêu cầu: Cho một xâu kí tự S chiều dài không quá 255 kí tự. Tìm mật khẩu P đã dấu trong S.
}

uses crt;
var S:string;
    i:integer;
    tong:longint;
Begin
  clrscr;
  write('Nhap S = '); readln(S);
  tong:=0;
  for i:=1 to length(S) do
    if (S[i]>'0') and (S[i]<='9') then
    //if S[i] in ['0'..'1'] then //not working
      tong:=tong+ORD(S[i])-48;
      Write('Mat khau la: ',tong);
readln;
End.
